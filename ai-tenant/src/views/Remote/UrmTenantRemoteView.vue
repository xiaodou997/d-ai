...
import {
  findUrmTenantPageByPath,
  getUrmTenantPageRemoteKey,
  getUrmTenantStandaloneUrl,
  loadUrmTenantManifest
} from '@/remote/urmTenantManifest'
import { loadUrmTenantRemoteComponent } from '@/remote/urmTenantRemote'
...
const remotePage = computed(() => getUrmTenantPageRemoteKey(activePage.value))
...
async function loadPage() {
  ...
  try {
    ...
    const page = findUrmTenantPageByPath(manifest, route.path)
    if (!page) {
      throw new Error(`URM manifest 未配置页面：${route.path}`)
    }

    activePage.value = page
    standaloneUrl.value = await getUrmTenantStandaloneUrl(standalonePath.value)
    RemoteComponent.value = markRaw(await loadUrmTenantRemoteComponent())
  } catch (err) {
    error.value = err?.message || 'URM 模块加载失败'
    if (!standaloneUrl.value) {
      standaloneUrl.value = await getUrmTenantStandaloneUrl(standalonePath.value).catch(() => '')
    }
  } finally {
    loading.value = false
  }
