...
func (s *Server) handleAdminGetRouteWeights(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) handleAdminPutRouteWeights(w http.ResponseWriter, r *http.Request) {
	scope := chi.URLParam(r, "scope")
	if scope == "" {
		writeErr(w, http.StatusBadRequest, BizErrBadRequest, "scope path param is required")
		return
