...
CREATE TABLE IF NOT EXISTS ai_providers (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (protocol_type IN ('openai_chat_completions', 'openai_images_generations', 'openai_responses', 'anthropic_messages'))
);
...
CREATE TABLE IF NOT EXISTS ai_model_deployments (
  ...
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (capability_type IN ('chat', 'image', 'video', 'embedding', 'audio', 'rerank')),
  CHECK (upstream_protocol IN ('openai_chat_completions', 'openai_images_generations', 'openai_responses', 'anthropic_messages')),
