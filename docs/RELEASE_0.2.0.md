...
> 本文只保留 0.2.0 的历史账务迁移记录，不是当前发布待办。当前 schema v27、迁移链、备份/恢复和上线检查统一以 [`SCHEMA_RELEASE_RUNBOOK.md`](SCHEMA_RELEASE_RUNBOOK.md)、[`DATABASE.md`](DATABASE.md) 和 [`RUNTIME_ROLES.md`](RUNTIME_ROLES.md) 为准；本文中的未勾选项不代表当前版本尚未完成。
...
2. 从上线前的备份恢复数据库
3. 启动 0.1.3
4. 用旧版消费流水与上游账单人工核对窗口内产生的消费，必要时补记
