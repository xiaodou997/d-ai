...
- `internal/db/init.sql` 是当前唯一完整结构，schema 版本为 `20`。
...
schema v19 修复历史 v1→v18 链遗漏的 `ai_usage_logs.billing_status` 索引，schema v20 增加 billing/payment 跨域只读投影视图。
...
`internal/db/changes/0020_20260826_cross_domain_read_models.sql`；不要直接修改
schema 版本号跳过脚本。
