...
- 数据库使用 `internal/db/init.sql` schema v19 完整基线；应用只校验 `dai_schema_metadata.version`，不执行生产迁移。
- `internal/db/changes/` 保存 v1→v19 的 forward-only 人工升级链，`cmd/checkschema` 校验连续版本、来源 guard、事务和基线版本，并在 CI 校验生成清单。
...
| 数据库 | `internal/db/init.sql`, `internal/db/changes`, `cmd/checkschema` | schema v19 新库基线、v1→v19 forward-only 人工变更和结构门禁 |
