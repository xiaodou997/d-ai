...
### P2-05（Tenant dashboard workspace boundary，2026-08-29）

- 垂直切分：租户 AI 概览的数据加载、时间范围刷新、资产/调用/质量聚合和导航工作区迁移到 `features/ai/dashboard/TenantDashboardWorkspace.vue`；指标、图表和质量子组件及 token 解析器一并归入 dashboard feature，`views/tenant/ai/DashboardView.vue` 收敛为路由 wrapper。
...
### P2-05（Admin-users workspace boundary，2026-08-29）

- 垂直切分：平台管理员账号筛选、创建/编辑、删除、密码重置和激活凭证提示工作区迁移到 `features/platform/user-management/AdminUsersWorkspace.vue`；`views/admin/platform/AdminUsersView.vue` 收敛为路由入口 wrapper，与终端用户工作区共享 feature 出口。
- 依赖边界：管理员工作区只组合 typed `AdminUserItem`、平台管理员 API、`useListPage`、`FormInstance` 和 DsUI 表格/分页，路由层不再持有管理员账号请求与表单编排。
- 回归：新增工作区列表加载与创建入口集成测试；定向测试、全量 Portal 105 个测试文件/332 项测试、typecheck、architecture check、frontend build 和全仓 Go 门禁通过。

### P2-05（Admin withdrawals workspace boundary，2026-08-29）
...
### P2-05（Admin business-overview workspace boundary，2026-08-29）

- 垂直切分：平台业务概览的时间范围、全局业务指标、调用趋势、热门模型、租户排行和失败数据提示迁移到 `features/admin/overview/BusinessOverviewWorkspace.vue`；`views/admin/overview/BusinessOverviewView.vue` 收敛为路由入口 wrapper。
...
### P2-05（Customer group-pricing adapter，2026-08-29）

- 边界：客户模型定价的分组/生效价格 facade、能力标签、服务导航和刷新通知适配迁移到 `features/ai/group-pricing/CustomerGroupPricingWorkspace.vue`；`views/customer/ai/GroupsView.vue` 仅保留路由入口。
...
- 敏感操作：管理员账号/租户/用户状态变更、密码重置、删除、退款等操作要求 Redis 中存在 30 分钟近期密码或 MFA 认证标记；新增 `POST /api/auth/recent-auth` 用于恢复该标记。
