...
- 订阅 HTTP 模块：`SubscriptionHTTPDeps` 独立组合六组订阅端口、`HTTPAuthDeps`、身份补全 provider 和失败 observer；`RegisterSubscriptions` 自行注册租户/终端用户认证分组。顶层仅通过 composition-only `AIHTTPDeps` 聚合 Core、订阅、风控、审计读取、系统、管理仪表盘和管理用量模块，路由 handler 不再接触聚合容器；契约测试确认 core 不注册订阅路径、独立模块注册后执行认证。
...
- 遗留风险：订阅、风控、审计读取、系统、管理仪表盘和管理用量路由已脱离 `AIDeps`，其余 AI core 路由仍共享巨型接口型 service locator；顶层平台 `OperationsDeps` 仍保留多项具体 service，部分 worker 只有 context 取消，没有可等待的 `Stop/Health` 接口。
- 下一候选项：P1-02 提取 OAuth pool/credential 管理 HTTP 模块；保留 serving 与 token refresh 的运行时端口，继续把密钥池管理认证和能力组合从 Core 中分离。
