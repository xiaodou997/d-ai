...
- [x] 空库基线由完整迁移链生成或验证，避免同时手工维护两套结构；`scripts/replay_schema_chain.sh` 从不可变 v1 Git 基线重放到 v20，并将最终 schema-only dump 与 `init.sql` 比较。
- [x] 每个迁移在空库和前一 schema 版本副本上验证；19 个迁移有真实 PostgreSQL 专项测试，CI 另执行 v1→v20 全链重放并逐步校验版本。
...
- [~] 跨域读取通过视图、只读端口或显式 query service；billing 充值和 payment 管理投影已迁移到只读视图，剩余租户/用户/运营聚合继续收敛。
