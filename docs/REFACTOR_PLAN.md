...
- [~] 清理 staticcheck 报告的死代码、无效赋值和潜在 nil dereference；已用与 Go 1.27 匹配的 staticcheck 清零 `internal/ai/adapters/postgres`、`internal/ai/adapters/bridgefmt`、`internal/ai/billingcontrol`、`internal/ai/clientruntime`、`internal/ai/console`、`internal/ai/core/runtime`、`internal/ai/domain`、`internal/ai/formats`、`internal/ai/gateway`、`internal/ai/serving`、`internal/ai/transport`、`internal/payment/wechat` 与 `internal/tenant/pg` 诊断，其他包仍待分批处理。
