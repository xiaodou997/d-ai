...
- AI 风控、审计、OAuth 刷新和结算 outbox worker 的 `Stop(ctx)` 现在返回 deadline 错误；`aiModules` 聚合并向 shutdown stack 传播关闭失败，后续关闭调用可继续等待而不会虚报已停止。
- Async task 与 Webhook worker 的 claim/heartbeat/reaper/终态写入均以 PostgreSQL lease owner fencing 为准；多副本并发 claim 与过期接管后的 stale writer 回归测试已覆盖。
...
- 公告发布、下线和草稿删除的状态前置检查已下沉到公告 PostgreSQL 事务；Service 不再先读公告再决定是否写入，状态转换在行锁内保持原子。
...
- 跨副本真相源审计已完成：任务、账务和租约状态只依赖 PostgreSQL/Redis durable state；Redis-backed health tracker 在共享 Redis 不可达时不使用进程内 circuit-breaker 状态，读取 fail-closed，避免不同副本分叉。
...
所有 HTTP 纵向模块均通过 `transport.Module` 或 `RawDeps` 注册，模块专属接口由组合根逐项投影；模块注册治理已完成。
