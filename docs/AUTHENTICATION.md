...
- 平台管理员可在个人中心注册 TOTP MFA。启用后密码登录只返回短时一次性 MFA 挑战，验证码验证成功后才签发 session；平台管理端和 AI 管理端的状态变更、密钥/支付/Provider/路由/风控配置写操作还要求 30 分钟内完成过密码或 MFA 近期认证。
- 修改密码、注册/确认 MFA 等高影响账号操作同样要求近期认证；近期认证标记在 Redis 不可用时拒绝放行。只读 GET/HEAD 查询不额外触发近期认证。
