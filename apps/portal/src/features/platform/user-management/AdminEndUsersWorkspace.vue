<!--
  Admin end-user management workspace.
...
type EndUserRow = EndUserItem & { statusDisplay: string }
...
const {
...
} = useListPage<{ tenantName: string; username: string; status: number | '' }, EndUserRow>({
  ...
  fetcher: async (params) => {
    try {
      ...
      return {
        items: (data.items || []).map((r): EndUserRow => ({
          ...r,
...
const handleStatusChange = async (row: EndUserRow, enabled: boolean) => {
