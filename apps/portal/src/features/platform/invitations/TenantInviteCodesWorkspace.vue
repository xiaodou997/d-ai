...
import type { InviteCodeItem } from "@/api/types/platformTenant";
...
const {
  ...
  fetcher: async (params) => {
    try {
      const res = await platformTenantApi.getInviteCodes({ page: params.page, size: params.pageSize });
...
async function handleCreate() {
  ...
  try {
    const created = await platformTenantApi.createInviteCode({
...
async function toggleStatus(row: InviteCodeItem) {
  ...
  try {
    ...
    await platformTenantApi.updateInviteCode(row.id, { status: newStatus });
...
async function handleDelete(row: InviteCodeItem) {
  try {
    ...
    await platformTenantApi.deleteInviteCode(row.id);
