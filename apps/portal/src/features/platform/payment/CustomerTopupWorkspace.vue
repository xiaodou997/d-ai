...
const columns: DsTableColumn[] = [
  ...
  { key: "gross", title: "充值基数", width: 130, align: "right" }, { key: "fee", title: "手续费", width: 120, align: "right" },
  { key: "credited", title: "到账", width: 130, align: "right" },
...
function packageFeeMicroUsd(pkg: TopupPackage) {
  if (!pkg.feeEnabled || !config.value) return 0;
  return Math.ceil((pkg.paymentAmountMicroUsd * config.value.feeRateBp) / 10_000);
}
...
function handleSuccess() { dialogVisible.value = false; activeOrder.value = null; amountUsd.value = null; selectedPackage.value = null; ElMessage.success("充值成功，USD 额度已到账"); void fetchList(); }
...
<template>
  <div class="topup-page">
    <PortalPagePanel fill :icon="CreditCard" :breadcrumbs="[{ label: '用户中心' }, { label: 'USD 账户' }, { label: '额度充值' }]" description="使用微信支付充值 USD 额度，订单明确记录支付、手续费和到账金额。">
      <div class="topup-body">
        ...
        <template v-else>
          <section class="topup-section">
            <header><h2>选择充值金额</h2><p>额度包按到账金额配置，可按规则选择是否收取手续费；自定义金额按支付金额扣除手续费。</p></header>
            ...
            <template v-else-if="config">
              ...
              <div v-if="packages.length" class="package-grid" aria-label="额度包">
                <button v-for="pkg in packages" :key="pkg.id" type="button" :class="['package-card', { 'is-active': selectedPackage?.id === pkg.id }]" @click="submitPackage(pkg)">
                  <span v-if="pkg.badge" class="package-badge">{{ pkg.badge }}</span><strong>到账 {{ formatMicroUSD(pkg.paymentAmountMicroUsd) }}</strong><em>{{ pkg.name }}</em>
          ...
          <section class="topup-section"><header><h2>充值订单</h2><p>订单保留支付、手续费和最终到账金额快照。</p></header>
...
<style scoped>
.topup-page { display:flex; flex:1; min-height:0; flex-direction:column; }.topup-body { display:flex; flex:1; min-height:0; flex-direction:column; gap:24px; padding:24px; }.topup-section { display:flex; flex-direction:column; gap:16px; }.topup-section+.topup-section { border-top:1px solid var(--ds-line); padding-top:24px; }.topup-section header { display:flex; align-items:baseline; gap:12px; }.topup-section h2 { margin:0; color:var(--ds-ink); font-size:14px; }.topup-section p { margin:0; color:var(--ds-muted); font-size:12px; }.topup-loading { min-height:160px; }.rule-bar { display:flex; flex-wrap:wrap; gap:8px 20px; border-left:3px solid var(--ds-accent); padding:10px 14px; background:var(--ds-panel-muted); color:var(--ds-muted); font-size:12px; }.rule-bar strong { color:var(--ds-ink); }.package-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:12px; }.package-card { position:relative; display:flex; min-height:130px; flex-direction:column; align-items:flex-start; gap:5px; overflow:hidden; border:1px solid var(--ds-line); border-radius:var(--ds-radius-control); padding:14px; background:var(--ds-panel); color:var(--ds-ink); cursor:pointer; text-align:left; }.package-card:hover,.package-card.is-active { border-color:var(--ds-accent); box-shadow: var(--ds-shadow-accent-outline); }.package-card strong { font-size:20px; }.package-card em { font-size:12px; font-style:normal; }.package-card small { color:var(--ds-muted); }.package-badge { position:absolute; top:0; right:0; padding:3px 7px; border-bottom-left-radius:var(--ds-radius-sm); background:var(--ds-accent-soft); color:var(--ds-accent); font-size:11px; }.custom-box { display:grid; grid-template-columns:minmax(150px,1fr) minmax(180px,1fr) minmax(240px,1.4fr) auto; align-items:center; gap:16px; border:1px solid var(--ds-line); border-radius:var(--ds-radius-control); padding:16px; background:var(--ds-panel-muted); }.custom-box p { margin-top:3px; }.custom-input { width:100%; }.custom-preview { display:flex; flex-direction:column; gap:2px; color:var(--ds-muted); font-size:12px; }.custom-preview strong,.amount-positive { color:var(--ds-positive); }.amount-positive { font-weight:700; }
