<!--
  Tenant user-recharge records workspace.
...
<template>
  <div class="page-container recharge-records-page">
    ...
    <el-dialog v-model="reverseDialogVisible" title="确认撤销充值" width="480" :close-on-click-modal="false" :append-to-body="true">
      <div class="space-y-4">
        <el-alert type="error" :closable="false" show-icon class="reverse-warning-alert" title="此操作将回收该充值对应额度包的剩余金额，请确认操作无误。" />
...
async function confirmReverse() {
  ...
  try {
    ...
    ElMessage.error(errorMessage(err, "撤销失败"));
  } finally {
