<!--
  管理端租户详情工作区 — 1:1 搬运自 v1/platform/platform-admin/src/views/Tenant/TenantDetail.vue。
  2 Tab：组织用户 / 关联用户。
...
import type { AdminUserItem, EndUserItem, TenantDetailOutput } from '@/api/types/admin'
...
const createOrgUserFormRef = ref<FormInstance>()
...
const handleToggleOrgUser = async (row: AdminUserItem, status: 'active' | 'disabled') => {
...
const handleResetOrgUserPwd = async (row: AdminUserItem) => {
  try {
    await ElMessageBox.confirm(
