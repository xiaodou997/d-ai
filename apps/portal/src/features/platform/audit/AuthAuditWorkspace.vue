<!-- Authentication audit workspace. -->
<template>
...
type PrincipalTone = 'accent' | 'warning' | 'neutral' | 'info'
type DecisionTone = 'positive' | 'danger' | 'warning' | 'info'

const principalLabels: Record<string, string> = { user: '用户', admin: '管理员' }
const principalTones: Record<string, PrincipalTone> = { user: 'accent', admin: 'neutral' }
const decisionLabels: Record<string, string> = { success: '通过', deny: '拒绝', error: '错误' }
const decisionTones: Record<string, DecisionTone> = { success: 'positive', deny: 'danger', error: 'warning' }

const principalLabel = (t: string) => principalLabels[t] || t
const principalTone = (t: string): PrincipalTone => principalTones[t] || 'info'
const decisionLabel = (d: string) => decisionLabels[d] || d
const decisionTone = (d: string): DecisionTone => decisionTones[d] || 'info'
</script>
