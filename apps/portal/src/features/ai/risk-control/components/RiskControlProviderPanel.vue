...
<template>
  <div class="provider-panel">
    ...
    <template v-else-if="config">
      ...
      <div class="provider-card"><div class="provider-heading"><div><h3>审核 API</h3><p>仅对选定的上游账号调用 Moderations 接口。</p></div><div class="test-actions"><DsTag :tone="config.provider.enabled ? 'positive' : 'info'">{{ config.provider.enabled ? config.provider.mode : '已关闭' }}</DsTag><RiskControlConfigPanel section="provider" /></div></div><dl><dt>Base URL</dt><dd>{{ config.provider.base_url || '-' }}</dd><dt>模型</dt><dd>{{ config.provider.model || '-' }}</dd><dt>API Key</dt><dd>{{ config.provider.has_api_key ? '已配置（不回显）' : '未配置' }}</dd></dl></div>
