...
import type { AuditLogDTO } from '@/api/types/ai'
...
const logs = shallowRef<AuditLogDTO[]>([])
...
const fetchLogs = async () => {
  ...
  try {
    ...
    const res = await aiAdminApi.listGatewayAuditLogs({
      limit: limit.value
    })
...
const resultTone = (result: string): 'positive' | 'danger' | 'info' => {
