...
async function probe(endpoint: PromptAuditEndpointWrite & { has_api_key?: boolean }) {
  try {
    // has_api_key is a response-only field and must never be sent to the write endpoint.
    const { has_api_key: _hasApiKey, ...payload } = endpoint
    const result = await probePromptAuditEndpoint(payload)
    result.ok ? toast.success('节点测试成功') : toast.error(`节点测试失败：${result.error_code || 'unknown'}`)
  } catch { toast.error('节点测试失败') }
}
