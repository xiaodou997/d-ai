...
describe("AI admin upstream account generated operation facade", () => {
  ...
  it("maps nullable transfer responses and forwards import/export contracts", async () => {
    mocks.request
      .mockResolvedValueOnce({ schema_version: 4, exported_at: "2026-08-28T00:00:00Z", contains_plaintext_api_keys: true, accounts: null })
      .mockResolvedValueOnce({
        items: [{ name: "Imported", base_url: "https://api.example.com", action: "create", model_binding_count: 0, warnings: null }],
    ...
    await expect(aiAdminApi.previewImportUpstreamAccounts({
      accounts: [{ name: "Imported", tenant_display_name: "Imported", tenant_access_mode: "public", base_url: "https://api.example.com", api_key: "secret", default_provider_family: "openai_compatible", status: "active" }]
  ...
  it("rejects an unknown import preview action at the domain boundary", async () => {
    mocks.request.mockResolvedValueOnce({
      items: [{ name: "Imported", base_url: "https://api.example.com", action: "merge", model_binding_count: 0 }],
