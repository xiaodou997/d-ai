<!--
  租户端 — 用户充值明细:查看本租户下终端用户的积分充值明细,支持撤销充值。
...
<template>
  <div class="page-container recharge-records-page">
    ...
    >
      ...
      >
        ...
        <template #cell-paidAmount="{ row }">
          <span class="recharge-records-amount">¥ {{ (row.paidAmount / 100).toFixed(2) }}</span>
        </template>
        <template #cell-creditAmount="{ row }">
          <span class="recharge-records-credits">+{{ row.creditAmount?.toLocaleString() }}</span>
        </template>
    ...
    <el-dialog v-model="reverseDialogVisible" title="确认撤销充值" width="480" :close-on-click-modal="false" :append-to-body="true">
      <div class="space-y-4">
        <el-alert type="danger" :closable="false" show-icon class="reverse-warning-alert" title="此操作将回收该充值对应的积分包剩余积分，请确认操作无误。" />
        <div v-if="reverseRow" class="space-y-2 text-sm">
          ...
          <div class="flex justify-between"><span class="recharge-records-dialog-label">到账积分</span><span class="font-bold">{{ (reverseRow.creditAmount || 0).toLocaleString() }}</span></div>
        </div>
...
const columns: DsTableColumn[] = [
  ...
  { key: "paidAmount", title: "实付金额", width: 120, align: "right" },
...
const {
  ...
  fetcher: async (params) => {
    try {
      ...
      const res = await urmTenantApi.getUserRechargeRecords({
        page: params.page,
...
async function confirmReverse() {
  ...
  try {
    const result = await urmTenantApi.reverseRecharge(reverseRow.value.orderId, { reason: reverseForm.reason });
