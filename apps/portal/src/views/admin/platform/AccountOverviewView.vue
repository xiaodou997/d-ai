<!--
  账户全景 — 1:1 搬运自 v1/platform/platform-admin/src/views/Finance/AccountOverview.vue。
  适配：account axios → platformAdminApi.getAccountBalance。echarts 积分构成饼图保留。
...
const handleSearch = async () => {
  ...
  loading.value = true
  try {
    const response = await platformAdminApi.getAccountBalance({
      accountType: queryForm.accountType,
      accountId: queryForm.accountId.trim(),
      detail: true
    })
