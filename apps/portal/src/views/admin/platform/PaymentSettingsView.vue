...
async function fetchPaymentSettings() {
  ...
  try {
    ...
    ruleForm.customValidityDays = settings.tenantCustomValidityDays ?? null;
...
async function submitPaymentSettings() {
  ...
  try {
    const body: PaymentGlobalSettings = {
      ...
      tenantCustomValidityDays: ruleForm.customValidityDays || null,
        ...
        <el-form v-loading="settingsLoading" :model="ruleForm" label-position="top" class="settings-form">
          <section class="settings-section">
            ...
            <div class="form-grid form-grid--3">
              ...
              <el-form-item label="自定义充值有效期（天）">
                <el-input-number v-model="ruleForm.customValidityDays" :min="1" :controls="false" clearable class="w-full" placeholder="留空为长期有效" />
              </el-form-item>
            </div>
            <div class="preview-row">
              <div class="preview-strip">
                <span class="preview-strip__tag">充值举例</span>
                自定义充值 ${{ customPreview.amount }}，充值金额 {{ customPreview.gross }}，手续费 {{ customPreview.fee }}，实际到账 {{ customPreview.net }}。
              </div>
              <div class="preview-strip">
                <span class="preview-strip__tag">提现举例</span>
                管理员创建 $100.00 提现记录，手续费 {{ ruleForm.withdrawFeePercent }}%，预计实际打款 ${{ (100 - (100 * ruleForm.withdrawFeePercent) / 100).toFixed(2) }}。
              </div>
            </div>
          </section>
