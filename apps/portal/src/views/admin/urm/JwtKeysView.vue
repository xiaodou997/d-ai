<!-- JWT 密钥管理 — 1:1 搬运自 v1/urm/urm-admin/src/views/System/JwtKeys.vue（api → urmAdminApi，dayjs → 本地格式化）
...
const fetchKeys = async () => {
  loading.value = true
  try {
    const res = await urmAdminApi.listJwtKeys()
    keys.value = res?.keys || []
  } catch {
...
const confirmRotate = async () => {
  try {
    ...
    rotating.value = true
    await urmAdminApi.rotateJwtKey()
