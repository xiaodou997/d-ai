<!-- 认证审计 — 1:1 搬运自 v1/urm/urm-admin/src/views/System/AuditLog.vue（getAuthAuditLogs，data.items）。
...
<template>
  <div class="audit-log-page">
    <PortalPagePanel
      ...
      description="查看用户登录、Token 刷新与 Service Token 签发/拒绝的认证审计流水。"
    >
      <template #filters>
        <DsFilterBar>
          <DsFilterField label="事件类型">
            <el-select v-model="query.eventType" placeholder="全部事件类型" clearable class="audit-log-select">
              ...
              <el-option label="Service Token 签发" value="service_token_issued" />
              <el-option label="Service Token 拒绝" value="service_token_denied" />
            </el-select>
          </DsFilterField>
          ...
          <DsFilterField label="Client ID">
            <el-input v-model="query.clientId" placeholder="Client ID" clearable class="audit-log-input" @keyup.enter="search" />
          </DsFilterField>
      ...
      >
        ...
        <template #cell-clientId="{ row }">
          <span v-if="row.clientId" class="audit-log-mono">{{ row.clientId }}</span>
...
const {
  ...
  { eventType: string; principalType: string; decision: string; clientId: string; userId: string },
  AuditLogItem
>({
  initialQuery: { eventType: '', principalType: '', decision: '', clientId: '', userId: '' },
  pageSize: 20,
  // 显式标注返回类型：catch 分支引用 total.value，避免与 total 的推断形成循环
  fetcher: async (params): Promise<{ items: AuditLogItem[]; total: number }> => {
    try {
      const data = await urmAdminApi.getAuthAuditLogs({
        page: params.page,
...
const principalLabel = (t: string) => (({ user: '用户', service: '服务', admin: '管理员' } as any)[t] || t)
const principalTone = (t: string): 'accent' | 'warning' | 'neutral' | 'info' =>
  (({ user: 'accent', service: 'warning', admin: 'neutral' } as any)[t] || 'info')
