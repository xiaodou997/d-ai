<!--
  终端用户 — 1:1 搬运自 v1/urm/urm-admin/src/views/User/UserList.vue。
  保留搜索条件和账号启停，债务状态仅供查看。
  仅适配：axios api → v4 强类型 client（urmAdminApi，列表字段 items）；跳转租户详情走新路由。
...
const {
  ...
  fetcher: async (params) => {
    try {
      const data = await urmAdminApi.listEndUsers({
...
const handleToggleStatus = async (row: any) => {
  ...
  try {
    ...
    await urmAdminApi.updateEndUserStatus(row.userId, targetStatus === 2 ? 'disabled' : 'active')
