<!--
  手动充值中心 — 1:1 搬运自 v1/urm/urm-admin/src/views/Finance/Recharge.vue。
  管理员仅对「租户账户」充值（accountType=1 / packageType=1）；完整页面（非抽屉）。
  适配：axios api → urmAdminApi（listTenants 远程搜索 / getAccountBalance / createRecharge）；
...
onMounted(async () => {
  ...
  if (tenantId && tenantName) {
    ...
    let credits = 0
    try {
      const accountData = await urmAdminApi.getAccountBalance({ accountType: 1, accountId: tenantId })
...
const handleRemoteSearch = async (query: string) => {
  ...
  try {
    const res = await urmAdminApi.listTenants({ keyword: query, page: 1, size: 10 })
...
const handleRecharge = async () => {
  ...
  await rechargeFormRef.value.validate(async (v: boolean) => {
    ...
    try {
      ...
      const result = await urmAdminApi.createRecharge({
