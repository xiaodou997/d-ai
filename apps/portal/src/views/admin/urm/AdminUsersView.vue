<!--
  平台管理员 — 1:1 搬运自 v1/urm/urm-admin/src/views/System/AdminUserList.vue。
  适配：system axios → urmAdminApi（listSystemAdmins/createSystemAdmin/updateSystemAdmin/
...
const {
  ...
  fetcher: async (params) => {
    try {
      const data = await urmAdminApi.listSystemAdmins({
...
const services = ref<Awaited<ReturnType<typeof urmAdminApi.listServices>>>([])
const portalServices = computed(() => services.value.filter((service) => service.portalEnabled))
const form = reactive({ userId: '', username: '', email: '', status: 1, serviceMode: 'all' as 'all' | 'selected', serviceIds: [] as string[] })
...
const handleCreate = () => {
  isEdit.value = false
  Object.assign(form, { userId: '', username: '', email: '', status: 1, serviceMode: 'all', serviceIds: [] })
  dialogVisible.value = true
}

const handleEdit = async (row: any) => {
  const policy = await urmAdminApi.getSystemAdminServiceAccess(row.userId)
  isEdit.value = true
  Object.assign(form, { userId: row.userId, username: row.username, email: row.email, status: row.status, serviceMode: policy.mode, serviceIds: [...policy.serviceIds] })
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      if (isEdit.value) {
        await urmAdminApi.updateSystemAdmin(form.userId, { email: form.email, status: form.status })
        await urmAdminApi.updateSystemAdminServiceAccess(form.userId, { mode: form.serviceMode, serviceIds: form.serviceIds })
        ElMessage.success('更新成功')
      } else {
        await urmAdminApi.createSystemAdmin({
...
const handleResetPassword = async (row: any) => {
  try {
    ...
    await urmAdminApi.updateSystemAdmin(row.userId, { email: row.email, status: row.status, password: '123456' })
...
const handleDelete = async (id: string) => {
  try {
    await urmAdminApi.deleteSystemAdmin(id)
...
onMounted(() => {
  void urmAdminApi.listServices().then((items) => { services.value = items })
})
</script>
