<!-- 充值记录 — 1:1 搬运自 v1/urm/urm-admin/src/views/Finance/RechargeRecords.vue（api → urmAdminApi，data.items）
...
const {
  ...
  fetcher: async (params) => {
    try {
      const data = await urmAdminApi.listRechargeRecords({
        page: params.page,
...
const confirmReverse = async () => {
  ...
  try {
    const result = await urmAdminApi.reverseRecharge(reverseRow.value.orderId, { reason: reverseForm.reason })
