<!--
  交易流水 — 1:1 搬运自 v1/urm/urm-admin/src/views/Finance/TransactionList.vue。
  保留多条件筛选、批量确认/免除/退款、单条退款/确认/免除、行展开 ops 时间线、结果弹窗。
  适配：account axios api → urmAdminApi（listTransactions/refund/manualConfirmEvent/adminDismissEvent/
...
const {
  ...
  fetcher: async (params) => {
    try {
      const res = await urmAdminApi.listTransactions({
        page: params.page,
