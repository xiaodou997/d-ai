...
const {
  ...
  fetcher: async (params) => {
    try {
      const res = await urmAdminApi.listCashAccounts({ page: params.page, size: params.pageSize });
...
async function openLedger(row: CashAccountItem) {
  ...
  try {
    const res = await urmAdminApi.listCashLedger({ tenantId: row.tenantId, page: 1, size: 50 });
