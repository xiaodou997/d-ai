<!--
  控制概览 — 1:1 搬运自 v1/urm/urm-admin/src/views/Dashboard/index.vue。
  适配：analytics axios api → urmAdminApi（getGlobalStats/getConsumptionTrend/
...
const fetchStats = async () => {
  const data = await urmAdminApi.getGlobalStats(buildRangeParams())
  Object.assign(stats, data)
}

const fetchAlerts = async () => {
  try {
    const data = await urmAdminApi.getDashboardAlerts()
...
const fetchTrend = async () => {
  ...
  try {
    ...
    const res = await urmAdminApi.getConsumptionTrend(params)
...
const fetchResource = async () => {
  ...
  try {
    const res = await urmAdminApi.getResourceStatistics(buildRangeParams())
