<!--
  租户管理 — 1:1 搬运自 v1/urm/urm-admin/src/views/Tenant/TenantList.vue。
  适配：axios api → urmAdminApi（listTenants/createTenant/updateTenant/deleteTenant/
  ...
  <div class="tenants-page">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑租户' : '创建租户'" width="560px" append-to-body>
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        ...
        <el-divider />
        <ServiceAccessEditor
          v-model:mode="form.serviceMode"
          v-model:service-ids="form.serviceIds"
          :services="editableServices"
          :allow-all="authStore.userType === 1"
        />
...
const availableApps = ref<Awaited<ReturnType<typeof urmAdminApi.listServices>>>([])
const editableServices = computed(() => availableApps.value.filter((service) =>
  service.portalEnabled && (authStore.userType === 1 || authStore.enabledClientIds.includes(service.serviceId))
))
...
const {
  ...
  fetcher: async (params) => {
    try {
      const data = await urmAdminApi.listTenants({
...
const form = reactive({
  ...
  contactEmail: '',
  status: 1,
  serviceMode: 'all' as 'all' | 'selected',
...
const handleCreate = () => {
  isEdit.value = false
  const policy = editablePolicyForActor({ mode: 'all', serviceIds: [] }, authStore.userType, authStore.enabledClientIds)
  Object.assign(form, { tenantId: '', tenantName: '', contactPerson: '', contactEmail: '', status: 1, serviceMode: policy.mode, serviceIds: policy.serviceIds, initUsername: '', initEmail: '' })
...
const loadApps = async () => {
  try {
	availableApps.value = await urmAdminApi.listServices()
  } catch {}
}

const handleEdit = async (row: any) => {
  const policy = await urmAdminApi.getTenantServiceAccess(row.tenantId)
  const editablePolicy = editablePolicyForActor(policy, authStore.userType, authStore.enabledClientIds)
  isEdit.value = true
  Object.assign(form, {
    ...
    status: row.status,
    serviceMode: editablePolicy.mode,
    serviceIds: editablePolicy.serviceIds,
    initUsername: '',
    initEmail: ''
  })
  dialogVisible.value = true
...
const handleToggleStatus = async (row: any) => {
  ...
  try {
    ...
    await urmAdminApi.updateTenantStatus(row.tenantId, isDisable ? 'disabled' : 'active')
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      if (isEdit.value) {
        await urmAdminApi.updateTenant(form.tenantId, {
          tenantName: form.tenantName,
          contactPerson: form.contactPerson || undefined,
          contactEmail: form.contactEmail || undefined,
          status: form.status
        })
        await urmAdminApi.updateTenantServiceAccess(form.tenantId, { mode: form.serviceMode, serviceIds: form.serviceIds })
        ElMessage.success('更新成功')
      } else {
        const body: any = {
          tenantName: form.tenantName,
          status: form.status,
          serviceAccess: { mode: form.serviceMode, serviceIds: form.serviceIds }
        }
        ...
        const res: any = await urmAdminApi.createTenant(body)
        if (res.initUsername) {
...
const handleDelete = async (id: string) => {
  try {
    await urmAdminApi.deleteTenant(id)
