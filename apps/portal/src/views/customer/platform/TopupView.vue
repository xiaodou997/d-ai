...
async function fetchConfig() {
  ...
  try {
    config.value = await platformCustomerApi.getTopupConfig();
  } catch (e) {
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await platformCustomerApi.listTopupOrders({ page: page.value, size: pageSize.value });
...
async function submitPackage(pkg: TopupPackage) {
  ...
  try {
    activeOrder.value = await platformCustomerApi.createTopupOrder({ packageId: pkg.id });
...
async function submitCustom() {
  ...
  submitting.value = true;
  try {
    activeOrder.value = await platformCustomerApi.createTopupOrder({ amount: Math.round(amountYuan.value * 100) });
    dialogVisible.value = true;
  } catch (err) {
    const e = err as { detail?: string; message?: string };
    ElMessage.error(e?.detail || e?.message || "发起充值失败");
...
async function pollActiveOrder(): Promise<QrPayPollResult> {
  ...
  const status = await platformCustomerApi.getTopupOrder(activeOrder.value.orderId);
