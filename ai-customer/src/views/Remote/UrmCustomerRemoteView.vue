...
import {
  findUrmCustomerPageByPath,
  getUrmCustomerPageRemoteKey,
  getUrmCustomerStandaloneUrl,
  loadUrmCustomerManifest
} from '@/remote/urmCustomerManifest'
import { loadUrmCustomerRemoteComponent } from '@/remote/urmCustomerRemote'
...
const remotePage = computed(() => getUrmCustomerPageRemoteKey(activePage.value))
const standalonePath = computed(() => activePage.value?.standalonePath || '/')
...
async function loadPage() {
  ...
  try {
    ...
    const page = findUrmCustomerPageByPath(manifest, route.path)
    if (!page) {
      throw new Error(`URM manifest 未配置页面：${route.path}`)
    }

    activePage.value = page
    standaloneUrl.value = await getUrmCustomerStandaloneUrl(standalonePath.value)
    RemoteComponent.value = markRaw(await loadUrmCustomerRemoteComponent())
  } catch (err) {
    error.value = err?.message || 'URM 模块加载失败'
    if (!standaloneUrl.value) {
      standaloneUrl.value = await getUrmCustomerStandaloneUrl(standalonePath.value).catch(() => '')
    }
  } finally {
    loading.value = false
  }
