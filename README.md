...
- 数据库使用 `internal/db/init.sql` schema v19 完整基线；已有数据库按 `internal/db/changes/` 的 v1→v19 forward-only 链显式升级，应用只校验版本，不执行自动迁移。
...
开发阶段使用低于当前 schema v19 的本地数据卷建议执行一次 `make db-recreate`；需要保留数据
