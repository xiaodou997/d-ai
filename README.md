...
- 数据库使用 `internal/db/init.sql` schema v31 完整基线；已有数据库按 `internal/db/changes/` 的 v1→v31 forward-only 链显式升级，应用只校验版本，不执行自动迁移。v26/v27 将路由策略归属分组并加入目标配置乐观并发版本，v31 支持公告删除审计。
...
开发阶段使用低于当前 schema v31 的本地数据卷建议执行一次 `make db-recreate`；需要保留数据
