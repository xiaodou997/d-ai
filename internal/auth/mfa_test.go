...
func TestMFACodeIsAcceptedOnlyOnceAcrossChallenges(t *testing.T) {
	ctx := context.Background()
	pool, cleanup, err := dbtest.OpenIsolatedSchemaPool(ctx, dbtest.PoolOptions{MaxConns: 4})
	if err != nil {
		t.Skipf("database unavailable: %v", err)
	...
	principal := Principal{UserID: "mfa-code-replay", Username: "mfa-replay-admin", UserType: 2, CredentialVersion: 1}
	if _, err := pool.Exec(ctx, `
		INSERT INTO iam_accounts (user_id, username, password_hash, user_type, status, mfa_secret_encrypted, mfa_enabled)
	...
	service := NewMFAService(pool, redisClient)
	first, err := service.CreateChallenge(ctx, principal)
	if err != nil {
		t.Fatal(err)
	}
	second, err := service.CreateChallenge(ctx, principal)
	if err != nil {
		t.Fatal(err)
	}
	code := totpCode(secret, uint64(time.Now().Unix()/30))
	...
	for verifyErr := range results {
		if verifyErr == nil {
			...
			t.Fatalf("unexpected cross-challenge MFA result: %v", verifyErr)
		}
	}
	if successes != 1 {
		t.Fatalf("same TOTP across challenges succeeded %d times, want 1", successes)
	}
}
