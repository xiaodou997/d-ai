...
type paymentNotifyHandlers struct {
	svc PaymentNotifyService
	log *zap.Logger
}

const maxWechatNotifyBodySize = 1 << 20

func newPaymentNotifyHandlers(svc PaymentNotifyService, logger *zap.Logger) *paymentNotifyHandlers {
	return &paymentNotifyHandlers{svc: svc, log: logger}
}
