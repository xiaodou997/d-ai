...
func TestRequestCutoverPreservesAssetsAndReleasesHistory(t *testing.T) {
 ...
 SELECT 'old-'||n,'openai_chat','/v1/chat/completions','model','failed',jsonb_build_object('content',(SELECT string_agg(md5(random()::text),'') FROM generate_series(1,1024))) FROM generate_series(1,500) n`)
	...
	migration44, err := os.ReadFile(filepath.Join(string(bytesTrimSpace(root)), "internal/db/changes/0044_20260918_jwt_active_key_invariant.sql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	migration46, err := os.ReadFile(filepath.Join(string(bytesTrimSpace(root)), "internal/db/changes/0046_20260919_auth_session_context_invariant.sql"))
	if err != nil {
		t.Fatal(err)
	}
	if _, err = pool.Exec(ctx, string(migration46)); err != nil {
		t.Fatal(err)
	}
