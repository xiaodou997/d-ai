...
var ErrEndUserNotFound = errors.New("urm end user not found")

// ErrInsufficientBalance 表示余额不足（或账户已透支）。
var ErrInsufficientBalance = errors.New("urm: insufficient balance")

// Claims 是 JWT claims 的类型别名——合并后直接复用 auth.Claims。
// 原 ai-service 从 urm-service 的 JWKS HTTP 端点拉公钥验签，
// 现在进程内直接用 auth.JWTService 验证，claims 结构完全一致。
...
type LegacyAuthorizationSettlementRequest struct {
...
type LegacyAuthorizationSettlementResponse = LedgerDebitReceipt
