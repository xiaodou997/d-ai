...
func TestRequestRuntimeSubjectReturnsCoreSubject(t *testing.T) {
	limit := int64(123)
	req := &Request{
	...
	if subject.APIKeyID != "key-1" {
		t.Fatalf("api key id = %q", subject.APIKeyID)
	}
	if subject.QuotaLimit == nil || *subject.QuotaLimit != 123 {
		t.Fatalf("quota limit = %#v", subject.QuotaLimit)
	}
	if subject.QuotaUsed != 11 {
		t.Fatalf("quota usage = %d", subject.QuotaUsed)
	}
}

// RouteCandidatesStep must preserve a structured *APIError (e.g. the 400
// no_matching_deployment returned by a route planner under strict
...
func TestRouteCandidatesStepWrapsGenericError(t *testing.T) {
	raw := errors.New("pq: password=secret internal_table=ai_routes")
	step := &RouteCandidatesStep{Selector: &fakeSelector{err: raw}}
	err := step.Execute(context.Background(), &Request{})
	var apiErr *APIError
	...
	if strings.Contains(apiErr.Message, "password") || strings.Contains(apiErr.Message, "ai_routes") {
		t.Fatalf("public route error leaked internal detail: %q", apiErr.Message)
	}
	req := &Request{}
	normalizePipelineError(req, &PipelineError{Step: step.Name(), Cause: err})
	if !strings.Contains(req.InternalErrorDetail, "internal_table=ai_routes") || strings.Contains(req.InternalErrorDetail, "password=secret") {
		t.Fatalf("internal detail was not retained with secret redaction: %q", req.InternalErrorDetail)
	}
}

func TestRouteCandidatesStepPreservesPrefilledCandidates(t *testing.T) {
	prefilled := &domain.RouteCandidate{
		RouteID:   "resolved-1",
		ModelCode: "gpt-5.4",
	}
	step := &RouteCandidatesStep{
		Selector: &fakeSelector{
			candidates: []*domain.RouteCandidate{{RouteID: "selector-1", ModelCode: "should-not-win"}},
		},
	}
	req := &Request{
		Candidates: []*domain.RouteCandidate{prefilled},
	}

	if err := step.Execute(context.Background(), req); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if len(req.Candidates) != 1 || req.Candidates[0].RouteID != "resolved-1" {
		t.Fatalf("prefilled candidates overwritten: %+v", req.Candidates)
	}
	if req.Candidate == nil || req.Candidate.RouteID != "resolved-1" {
		t.Fatalf("current candidate = %+v, want resolved-1", req.Candidate)
	}
	if req.UsedCandidates == nil {
		t.Fatal("expected used candidate map to be initialized")
	}
}

func TestRouteCandidatesStepFiltersImageRoutesByOutputCount(t *testing.T) {
	step := &RouteCandidatesStep{Selector: &fakeSelector{candidates: []*domain.RouteCandidate{
		{RouteID: "single", ImageMaxOutputCount: 1},
		{RouteID: "multi", ImageMaxOutputCount: 4},
	}}}
	req := &Request{CapabilityType: domain.CapabilityImage, ClientPath: "/v1/images/generations"}
	req.TokenUsage.ImageCount = 3
	if err := step.Execute(context.Background(), req); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if len(req.Candidates) != 1 || req.Candidates[0].RouteID != "multi" {
		t.Fatalf("eligible candidates = %+v", req.Candidates)
	}
}

func TestRouteCandidatesStepRejectsUnsupportedImageEditCount(t *testing.T) {
	step := &RouteCandidatesStep{Selector: &fakeSelector{candidates: []*domain.RouteCandidate{
		{RouteID: "generation-only", ImageMaxOutputCount: 4, ImageEditMaxOutputCount: 1},
	}}}
	req := &Request{CapabilityType: domain.CapabilityImage, ClientPath: "/v1/images/edits"}
	req.TokenUsage.ImageCount = 2
	err := step.Execute(context.Background(), req)
	var apiErr *APIError
	if !errors.As(err, &apiErr) || apiErr.Code != "image_count_not_supported" || apiErr.Status != http.StatusBadRequest {
...
type fakeAuditSubmitter struct {
	payload *audit.Payload
}

func (f *fakeAuditSubmitter) Submit(p *audit.Payload) bool {
	f.payload = p
	return true
}
...
func TestRequestSetCandidatePreservesRequestedModelAndSelection(t *testing.T) {
	req := &Request{RequestedModel: "claude-opus-4-1"}
	cand := &domain.RouteCandidate{
		RouteID:                      "route-1",
		ModelCode:                    "gpt-5.4",
		Protocol:                     domain.ProtocolOpenAIChat,
		UpstreamModel:                "gpt-5.4-upstream",
		MatchedDispatchRuleID:        "rule-1",
		MatchedDispatchRuleSummary:   "anthropic_messages exact claude-opus-4-1 -> gpt-5.4",
		ResolvedProviderFamily:       "openai_compatible",
		GroupAllowProtocolConversion: true,
		EndpointID:                   "acc-1",
	}

	req.SetCandidate(cand)

	if req.PublicModel() != "claude-opus-4-1" {
		t.Fatalf("public model = %q, want claude-opus-4-1", req.PublicModel())
	}
	if req.StickyModelKey() != "claude-opus-4-1" {
		t.Fatalf("sticky model key = %q, want claude-opus-4-1", req.StickyModelKey())
	}
	if req.ModelCode != "gpt-5.4" {
		t.Fatalf("resolved model code = %q, want gpt-5.4", req.ModelCode)
	}
	if req.SelectedUpstreamProtocol != string(domain.ProtocolOpenAIChat) {
		t.Fatalf("selected upstream protocol = %q", req.SelectedUpstreamProtocol)
	}
	if req.SelectedUpstreamModel != "gpt-5.4-upstream" {
		t.Fatalf("selected upstream model = %q", req.SelectedUpstreamModel)
	}
	if !req.UpstreamModelMappingApplied {
		t.Fatal("expected upstream model mapping to be recorded as applied")
	}
}

func TestAuditFinalizerIncludesDispatchObservability(t *testing.T) {
	submitter := &fakeAuditSubmitter{}
	finalizer := &AuditFinalizer{Worker: submitter}
	httpReq := httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	httpReq.Header.Set("Authorization", "Bearer sk-test")
	httpReq.Header.Set("User-Agent", "Claude-Code")
	req := &Request{
		Envelope: &RequestEnvelope{
			R:          httpReq,
			ClientBody: []byte(`{"model":"claude-opus-4-1","messages":[{"role":"user","content":"hello"}]}`),
		},
		RequestID:                   "req-1",
		ClientProtocol:              domain.ProtocolAnthropicMessages,
		RequestedModel:              "claude-opus-4-1",
		ModelCode:                   "gpt-5.4",
		MatchedDispatchRuleID:       "rule-1",
		MatchedDispatchRuleSummary:  "anthropic_messages exact claude-opus-4-1 -> gpt-5.4",
		ResolvedLogicalModel:        "gpt-5.4",
		ResolvedProviderFamily:      "openai_compatible",
		ProtocolConversionEnabled:   true,
		SelectedUpstreamProtocol:    string(domain.ProtocolOpenAIChat),
		SelectedUpstreamModel:       "gpt-5.4-upstream",
		UpstreamModelMappingApplied: true,
		RequestStatus:               domain.RequestSuccess,
		HTTPStatus:                  http.StatusOK,
	}

	finalizer.Finalize(context.Background(), req)

	if submitter.payload == nil {
		t.Fatal("expected audit payload")
	}
	if submitter.payload.RequestModel != "claude-opus-4-1" {
		t.Fatalf("request model = %q, want claude-opus-4-1", submitter.payload.RequestModel)
	}
	if submitter.payload.MatchedDispatchRuleID != "rule-1" {
		t.Fatalf("matched rule id = %q, want rule-1", submitter.payload.MatchedDispatchRuleID)
	}
	if submitter.payload.ResolvedLogicalModel != "gpt-5.4" {
		t.Fatalf("resolved logical model = %q, want gpt-5.4", submitter.payload.ResolvedLogicalModel)
	}
	if submitter.payload.SelectedUpstreamProtocol != string(domain.ProtocolOpenAIChat) {
		t.Fatalf("selected upstream protocol = %q", submitter.payload.SelectedUpstreamProtocol)
	}
	if submitter.payload.SelectedUpstreamModel != "gpt-5.4-upstream" {
		t.Fatalf("selected upstream model = %q", submitter.payload.SelectedUpstreamModel)
	}
	if !submitter.payload.ProtocolConversionEnabled {
		t.Fatal("expected protocol conversion enabled in audit payload")
	}
	if !submitter.payload.UpstreamModelMappingApplied {
		t.Fatal("expected upstream model mapping flag in audit payload")
	}
}
