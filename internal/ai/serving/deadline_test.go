...
func TestDeadlineControllerResponseHeaderTimeout(t *testing.T) {
	dc := newDeadlineController(context.Background(), domain.RouteTimeouts{
		ResponseHeader: 20 * time.Millisecond, FirstByte: time.Hour, Idle: time.Hour, MaxDuration: time.Hour,
	})
	defer dc.stop()
	waitCancel(t, dc, ErrResponseHeaderTimeout, time.Second)
}
...
func TestDeadlineControllerIdleResetThenTimeout(t *testing.T) {
	dc := newDeadlineController(context.Background(), domain.RouteTimeouts{
		ResponseHeader: time.Hour, FirstByte: time.Hour, Idle: 40 * time.Millisecond, MaxDuration: time.Hour,
	})
	defer dc.stop()
	dc.headersReceived()
	dc.firstByte()
...
func TestDeadlineControllerMaxDuration(t *testing.T) {
	dc := newDeadlineController(context.Background(), domain.RouteTimeouts{
		ResponseHeader: time.Hour, FirstByte: time.Hour, Idle: time.Hour, MaxDuration: 25 * time.Millisecond,
	})
	defer dc.stop()
	dc.headersReceived()
	dc.firstByte()
...
func TestDeadlineControllerMaxDurationIncludesConnectPhase(t *testing.T) {
	dc := newDeadlineController(context.Background(), domain.RouteTimeouts{
		ResponseHeader: time.Hour, FirstByte: time.Hour, Idle: time.Hour, MaxDuration: 25 * time.Millisecond,
	})
	defer dc.stop()
