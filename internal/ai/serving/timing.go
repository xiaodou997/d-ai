...
// MarkFirstOutput measures from the current upstream send to its first
// effective output. Setup and earlier attempts belong only to RequestTotalMs.
func (r *Request) MarkFirstOutput(at, attemptStart time.Time) {
	if r == nil || r.FirstTokenMs != 0 {
		return
	}
	r.FirstTokenMs = max(1, durationMs(attemptStart, at))
