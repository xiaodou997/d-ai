...
const (
	...
	defaultProbeLease       = maxOpenDuration
)
...
// Use NewRedisHealthTracker for a durable multi-node state machine.
...
func (t *InMemoryTracker) IsBlocked(targetID string, probeLease time.Duration) bool {
	if probeLease <= 0 {
	...
	case StateOpen:
		...
		e.probing = true
		e.probeUntil = time.Now().Add(probeLease)
		return false
	case StateHalfOpen:
		if e.probing && time.Now().Before(e.probeUntil) {
...
func (t *InMemoryTracker) ReleaseProbe(targetID string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	e, ok := t.entries[targetID]
...
func (t *InMemoryTracker) StatesOf(targetIDs []string) map[string]HealthState {
	t.mu.Lock()
	defer t.mu.Unlock()
	out := make(map[string]HealthState, len(targetIDs))
	for _, targetID := range targetIDs {
		if e, ok := t.entries[targetID]; ok {
			out[targetID] = e.state
		} else {
