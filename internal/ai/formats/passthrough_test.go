...
func TestExtractSyncUsage_Anthropic(t *testing.T) {
	...
	if u.PromptTokens != 780 || u.CompletionTokens != 25 || u.CacheWriteTokens != 200 || u.CacheReadTokens != 500 {
...
func TestExtractSyncUsage_Gemini(t *testing.T) {
	// Gemini reports thoughtsTokenCount separately from candidatesTokenCount, so
	// CompletionTokens must fold both in (15+5=20) to bill thinking tokens at the
	// output rate — otherwise the platform pays upstream for thoughts but never
	// charges them. ReasoningTokens stays a subset of CompletionTokens.
	...
	if u.PromptTokens != 60 || u.CompletionTokens != 20 || u.CacheReadTokens != 20 || u.ReasoningTokens != 5 {
		t.Errorf("got %+v — Gemini thoughts tokens must be folded into CompletionTokens", u)
	}
	if u.ReasoningTokens > u.CompletionTokens {
		t.Errorf("invariant violated: ReasoningTokens(%d) must be a subset of CompletionTokens(%d)", u.ReasoningTokens, u.CompletionTokens)
	}
}
...
func TestExtractStreamUsage_AnthropicSplit(t *testing.T) {
	...
	if u.PromptTokens != 342 || u.CacheReadTokens != 300 {
...
func TestExtractStreamUsage_AnthropicTerminalSnapshotMergesPresentFields(t *testing.T) {
	start := []byte(`{"type":"message_start","message":{"usage":{"input_tokens":0,"output_tokens":0,"cache_read_input_tokens":300}}}`)
	u, ok := ExtractStreamUsage(domain.TokenUsage{}, start, "message_start", domain.ProtocolAnthropicMessages)
	if !ok {
		t.Fatal("expected message_start to report usage")
	}
	delta := []byte(`{"type":"message_delta","usage":{"input_tokens":544,"output_tokens":55,"cache_creation_input_tokens":10}}`)
	u, ok = ExtractStreamUsage(u, delta, "message_delta", domain.ProtocolAnthropicMessages)
	if !ok {
		t.Fatal("expected message_delta to report usage")
	}
	if u.PromptTokens != 854 || u.CompletionTokens != 55 || u.CacheWriteTokens != 10 || u.CacheReadTokens != 300 {
		t.Fatalf("terminal usage = %+v, want prompt=854 output=55 cache-write=10 cache-read=300", u)
	}
}
