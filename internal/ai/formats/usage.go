...
func ExtractSyncUsage(body []byte, protocol domain.UpstreamProtocol) domain.TokenUsage {
	...
	default: // openai_chat, openai_embeddings, openai_images
...
func (u anthropicUsage) toDomain() domain.TokenUsage {
	// Anthropic reports uncached input separately; TokenUsage.PromptTokens is
	// the total input context and includes both cache creation and cache reads.
	promptTokens := tokenValue(u.InputTokens) + tokenValue(u.CacheCreationInputTokens) + tokenValue(u.CacheReadInputTokens)
	return domain.TokenUsage{
		PromptTokens:     promptTokens,
		CompletionTokens: tokenValue(u.OutputTokens),
		CacheWriteTokens: tokenValue(u.CacheCreationInputTokens),
		CacheReadTokens:  tokenValue(u.CacheReadInputTokens),
	}
}
...
// mergeAnthropicStreamUsage handles the split-stream model. Anthropic-native
// streams normally put input/cache counters in message_start and cumulative
// output_tokens in message_delta. Some compatible upstreams send provisional
// 0/0 usage in message_start, then publish the authoritative input/cache/output
// snapshot in message_delta, so every field present in the terminal event wins.
func mergeAnthropicStreamUsage(prev domain.TokenUsage, data []byte, eventType string) (domain.TokenUsage, bool) {
	...
	case "message_start":
		...
		return mergeAnthropicUsage(prev, *env.Message.Usage, false), true
	...
	case "message_delta":
		...
		return mergeAnthropicUsage(prev, *env.Usage, true), true
	}
	return prev, false
}

// mergeAnthropicUsage applies only fields that occurred in the event. Keeping
// pointer presence lets a terminal message_delta supplement a provisional
// message_start without erasing cache counters that the terminal event omits.
func mergeAnthropicUsage(prev domain.TokenUsage, update anthropicUsage, terminal bool) domain.TokenUsage {
	out := prev
	cacheWrite := prev.CacheWriteTokens
	cacheRead := prev.CacheReadTokens
	...
	if update.InputTokens != nil {
		out.PromptTokens = *update.InputTokens + cacheWrite + cacheRead
	} else if update.CacheCreationInputTokens != nil || update.CacheReadInputTokens != nil {
		uncachedInput := prev.PromptTokens - prev.CacheWriteTokens - prev.CacheReadTokens
		if uncachedInput < 0 {
			uncachedInput = 0
		}
		out.PromptTokens = uncachedInput + cacheWrite + cacheRead
	}
	out.CacheWriteTokens = cacheWrite
	out.CacheReadTokens = cacheRead
	if update.OutputTokens != nil && (terminal || *update.OutputTokens >= prev.CompletionTokens) {
		out.CompletionTokens = *update.OutputTokens
	}
	return out
}
...
func (u geminiUsage) toDomain() domain.TokenUsage {
	...
	// candidatesTokenCount (totalTokenCount = prompt + candidates + thoughts),
	// unlike OpenAI/Anthropic where reasoning is already folded into the output
	// count. Fold thoughts into CompletionTokens so extended-thinking tokens bill
	// at the output rate and the domain invariant holds (ReasoningTokens is a
	// subset of CompletionTokens; see domain.TokenUsage). This also matches the
	// canonical stream converter (geminiUsageToCanonical), so passthrough and
	// cross-protocol paths bill the same Gemini response identically.
	return domain.TokenUsage{
