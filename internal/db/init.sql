...
-- Immutable evidence for every operator-initiated financial repair. The
-- before/after snapshots are deliberately JSON rather than a mutable mirror
-- of business tables: the repair record says what was observed and changed,
-- while the source rows remain the authorities for current state.
...
ON ai_group_targets (group_id, priority);
  ...
  -- Requeue one parked outbox row only after an operator has repaired its root
  -- cause. The function writes the immutable audit evidence and restores the
  -- usage/outbox pair atomically; the original request_id remains the
  -- idempotency anchor.
  ...
  CREATE INDEX IF NOT EXISTS idx_ai_content_moderation_logs_input_hash ON ai_content_moderation_logs (input_hash) WHERE input_hash <> '';

  CREATE TABLE IF NOT EXISTS ai_prompt_audit_events (
    id                  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    request_id          TEXT,
    tenant_id           TEXT,
    user_id             TEXT,
  ...
  CREATE INDEX IF NOT EXISTS idx_ai_prompt_audit_events_created ON ai_prompt_audit_events (created_at DESC, id DESC);
  CREATE INDEX IF NOT EXISTS idx_ai_prompt_audit_events_tenant ON ai_prompt_audit_events (tenant_id, created_at DESC);
  CREATE INDEX IF NOT EXISTS idx_ai_prompt_audit_events_user ON ai_prompt_audit_events (user_id, created_at DESC);
  CREATE INDEX IF NOT EXISTS idx_ai_prompt_audit_events_decision ON ai_prompt_audit_events (decision, created_at DESC);
  CREATE INDEX IF NOT EXISTS idx_ai_prompt_audit_events_hash ON ai_prompt_audit_events (prompt_hash);
...
INSERT INTO bill_record_control(singleton) VALUES(true);
...
INSERT INTO dai_schema_metadata (singleton, version) VALUES (TRUE, 42);
