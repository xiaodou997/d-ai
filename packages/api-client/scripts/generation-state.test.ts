...
describe("OpenAPI generated type freshness", () => {
  it("requires generation on a fresh checkout", () => {
    const hash = generationHash([{ name: "ai-service", content: "openapi: 3.1.0" }]);
  ...
  it("accepts matching outputs and invalidates a stale specification", () => {
    const original = generationHash([{ name: "ai-service", content: "openapi: 3.1.0" }]);
    const changed = generationHash([{ name: "ai-service", content: "openapi: 3.1.0\ninfo: changed" }]);
