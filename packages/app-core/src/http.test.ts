...
describe("service access recovery", () => {
  it("does not resolve the auth store while creating a request adapter", async () => {
    ...
    const useAuthStore = vi.fn(() => authStore);
    const context = createPortalRequestContext({
      env: {
        portal: "admin",
        clientTypeHeader: "admin",
        xClientId: "urm-client",
  ...
  it("does not start SSO or issue a request for a service the tenant cannot access", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const context = createPortalRequestContext({
      env: {
        portal: "tenant",
        clientTypeHeader: "tenant",
        xClientId: "urm-client",
        serviceClientIds: { urm: "urm-client", proxy: "proxy-client" },
        urmBaseUrl: "http://urm",
        aiBaseUrl: "http://ai",
        aiPublicBaseUrl: "http://ai",
        proxyBaseUrl: "http://proxy",
    ...
    await expect(
      context.authenticatedRequest("proxy")({
        method: "GET",
        path: "/api/v1/consumption-stats",
        baseUrl: "http://proxy"
      })
    ).rejects.toEqual(new ServiceAccessUnavailableError("proxy"));
