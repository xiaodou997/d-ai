...
func ExtractSyncResponseMessage(body []byte, protocol domain.UpstreamProtocol) json.RawMessage {
	...
	case domain.ProtocolOpenAICompletions:
		return extractOpenAICompletionsMessage(body)
...
func extractOpenAICompletionsMessage(body []byte) json.RawMessage {
	var r struct {
		Choices []struct {
	...
	out, _ := json.Marshal(map[string]string{"role": "assistant", "text": r.Choices[0].Text})
	return out
}
