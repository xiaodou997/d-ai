...
func (s *Console) handleConsoleChatModelsV2(w http.ResponseWriter, r *http.Request) {
...
func (s *Console) handleConsoleChatListSessionsV2(w http.ResponseWriter, r *http.Request) {
	identity, ok := s.consoleRuntimeIdentity(w, r)
	if !ok {
		return
	}
...
func (s *Console) handleConsoleChatCreateSessionV2(w http.ResponseWriter, r *http.Request) {
	identity, ok := s.consoleRuntimeIdentity(w, r)
	if !ok {
		return
	}
...
func (s *Console) handleConsoleChatGetSessionV2(w http.ResponseWriter, r *http.Request) {
	identity, ok := s.consoleRuntimeIdentity(w, r)
	if !ok {
		return
	}
...
func (s *Console) handleConsoleChatDeleteSessionV2(w http.ResponseWriter, r *http.Request) {
	identity, ok := s.consoleRuntimeIdentity(w, r)
	if !ok {
		return
	}
...
func (s *Console) handleConsoleChatStreamV2(w http.ResponseWriter, r *http.Request) {
	identity, ok := s.consoleRuntimeIdentity(w, r)
	if !ok {
		return
	}
	...
	runtimeReq := s.gateway.ServeRuntime(capture, r, domain.CapabilityChat, gateway.RuntimeOverride{
...
func (s *Console) consoleGrantedChatModels(r *http.Request, identity *domain.RuntimeIdentity) ([]consoleChatModelRow, error) {
...
func (s *Console) consoleGrantedChatModelByCode(r *http.Request, identity *domain.RuntimeIdentity, modelCode string) (consoleChatModelRow, error) {
	models, err := s.consoleGrantedChatModels(r, identity)
	if err != nil {
...
func (s *Console) availableConsoleChatProtocols(r *http.Request, modelID pgtype.UUID) ([]domain.UpstreamProtocol, error) {
...
func (s *Console) modelHasConsoleProtocolRoute(r *http.Request, modelID pgtype.UUID, protocol domain.UpstreamProtocol) (bool, error) {
...
func (s *Console) chooseConsoleChatProtocol(r *http.Request, modelID pgtype.UUID, policy string, requested string) (domain.UpstreamProtocol, error) {
	if policy == "" {
		policy = "auto"
	}
	if policy == "manual" {
...
func (s *Console) loadConsoleChatSession(w http.ResponseWriter, r *http.Request, identity *domain.RuntimeIdentity, sessionID string) (consoleChatSessionDTO, bool) {
	var dto consoleChatSessionDTO
...
func (s *Console) listConsoleChatMessages(r *http.Request, sessionID string) ([]consoleChatMessageDTO, error) {
	rows, err := s.postgres.Query(r.Context(), `
...
func (s *Console) insertConsoleChatMessage(r *http.Request, sessionID string, role string, content string, protocol string, routeID string, usage any, errorInfo any) error {
...
func (s *Console) updateConsoleChatSessionRoute(r *http.Request, sessionID string, protocol domain.UpstreamProtocol, routeID string) error {
	var route any
	if routeID != "" {
		route = routeID
	}
