...
func (s *Console) handleAdminFetchEndpointUpstreamModels(w http.ResponseWriter, r *http.Request) {
...
func (s *Console) handleAdminImportEndpointUpstreamModels(w http.ResponseWriter, r *http.Request) {
	providerID, ok := parseUUIDParam(w, r, "providerID")
	if !ok {
		return
	}
	...
	for _, m := range req.Models {
		...
		tag, insertErr := s.postgres.Exec(r.Context(), `
			...
			ON CONFLICT (endpoint_id, credential_pool_id, upstream_model, upstream_protocol) DO NOTHING`,
...
func (s *Console) fetchUpstreamModelList(ctx context.Context, baseURL, apiKey, defaultProtocol string, extraHeaders []byte, timeoutMs int) ([]discoveredModel, error) {
