...
// writeServiceErr maps a service-layer error onto the management {code,message}
// envelope. It checks domain sentinels first, then falls back to the same
// pgx/pgconn mapping the legacy handlers used via writeDBErr (so repos that
// pass raw DB errors through still produce the correct 404/409/400 instead of a
// blanket 500). Truly unknown errors are logged and returned as a generic 500.
func (s *Console) writeServiceErr(w http.ResponseWriter, r *http.Request, err error) {
	...
	// Fallback: preserve the legacy writeDBErr contract for repos that surface
