...
// Shared helpers for mapping service-layer domain values back onto the legacy
// wire DTOs (which embed pgtype values). Reconstructing pgtype keeps the JSON
// output byte-identical to the pre-refactor mappers.
...
// matching the legacy millis(pgtype.Timestamptz) behaviour for NULL timestamps.
...
// matching the legacy millis(pgtype.Timestamptz) behaviour.
