<template>
  <div class="bg-white rounded-3xl border border-slate-50 shadow-soft overflow-hidden h-full flex flex-col">
    <div class="px-6 py-5 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10">
      ...
      <el-tag 
        :type="alertCount > 0 ? 'danger' : 'success'" 
        effect="plain" 
        class="border-none bg-slate-50 text-xs font-bold rounded-lg px-3"
      >
        {{ alertCount }} 项待审
      </el-tag>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
      ...
      <!-- 交易失败警示 -->
      <div v-for="item in failedTransactions" :key="item.transactionId"
        class="group p-4 bg-white hover:bg-amber-50/30 border border-slate-50 rounded-2xl transition-all relative overflow-hidden"
      >
        ...
        <div class="flex items-start justify-between mb-1">
          <div class="flex items-center">
            ...
            <div class="overflow-hidden">
              <p class="text-sm font-bold text-slate-700 truncate">扣费失败: {{ item.transactionId }}</p>
        ...
        <div class="mt-2 bg-slate-50 p-2 rounded-lg">
          <p class="text-xs text-slate-500 leading-relaxed font-medium">
            <span class="text-amber-600 font-bold">原因:</span> {{ item.failureReason }}
          </p>
        </div>
      </div>

      <!-- 超时预授权 -->
      <div v-for="item in timeoutPreAuths" :key="item.transactionId"
        class="group p-4 bg-white hover:bg-primary-50/30 border border-slate-50 rounded-2xl transition-all relative overflow-hidden"
      >
        ...
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center">
            ...
            <span class="text-sm font-bold text-slate-700">授权超时: {{ item.transactionId }}</span>
          </div>
        </div>
        <div class="flex justify-between items-center text-[11px]">
          <p class="text-slate-400 font-medium italic">于 {{ formatDateTime(item.createdTime) }} 创建</p>
          <el-tag size="small" class="rounded-md font-bold">等待处理</el-tag>
        </div>
      </div>
    </div>
  </div>
...
const alertCount = computed(() => {
  return props.timeoutPreAuths.length + props.failedTransactions.length
})
